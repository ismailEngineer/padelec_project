...
#define NRF_CSN_HIGH()  HAL_GPIO_WritePin(CSN_GPIO_Port, CSN_Pin, GPIO_PIN_SET)
#define NRF_CSN_LOW()   HAL_GPIO_WritePin(CSN_GPIO_Port, CSN_Pin, GPIO_PIN_RESET)
/* USER CODE END PV */
...
bool set_pwm(int channel, int percent);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(Disable_Driver_Mos_GPIO_Port, Disable_Driver_Mos_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin : Disable_Driver_Mos_Pin */
  GPIO_InitStruct.Pin = Disable_Driver_Mos_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(Disable_Driver_Mos_GPIO_Port, &GPIO_InitStruct);
...
bool set_pwm(int channel, int percent)
{
    ...
    uint32_t pulse = (htim4.Init.Period * percent) / 100;

    if (channel == 1) {
        __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, pulse );
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, htim4.Init.Period - pulse);
        return true;
    }
    else if (channel == 2) {
        __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_2, pulse );
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, htim4.Init.Period - pulse);
        return true;
     }
    else if (channel == 3) {
        __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_3, pulse );
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_3, htim4.Init.Period - pulse);
        return true;
     }
    return false;
}
